"""
数据迁移脚本：将 raw_msg 字段移到 content 字段后面
"""
...
def reorder_messages_table():
    """重新排列 messages 表字段顺序，将 raw_msg 移到 content 后面"""
    ...
    try:
        ...
        # 创建临时表，使用正确的字段顺序（raw_msg 在 content 后面）
        print("\n创建临时表...")
        cursor.execute('''
        ...
        cursor.execute('''
            ...
            (id, msg_id, from_wxid, to_wxid, room_wxid, content, raw_msg, wx_type, is_group, timestamp, create_time, extra)
            SELECT 
                id, 
                msg_id, 
                from_wxid, 
