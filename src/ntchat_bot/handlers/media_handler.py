...
def on_recv_image_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群图片消息] 群ID: {roomid}, 发送者: {from_wxid}, 图片路径: {image_path}")
        else:
...
def on_recv_voice_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群语音消息] 群ID: {roomid}, 发送者: {from_wxid}, 语音路径: {voice_path}")
        else:
...
def on_recv_video_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群视频消息] 群ID: {roomid}, 发送者: {from_wxid}, 视频路径: {video_path}")
        else:
...
def on_recv_emoji_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群表情消息] 群ID: {roomid}, 发送者: {from_wxid}")
        else:
...
def on_recv_location_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群位置消息] 群ID: {roomid}, 发送者: {from_wxid}, 位置: {location}, 坐标: {latitude},{longitude}")
        else:
...
def on_recv_link_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群链接消息] 群ID: {roomid}, 发送者: {from_wxid}, 标题: {title}, 描述: {description}, 链接: {url}")
        else:
...
def on_recv_file_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群文件消息] 群ID: {roomid}, 发送者: {from_wxid}, 文件名: {file_name}, 文件路径: {file_path}")
        else:
...
def on_recv_card_msg(wechat_instance: ntchat.WeChat, message):
    ...
    if from_wxid != self_wxid:
        if is_group:
            print(f"[群名片消息] 群ID: {roomid}, 发送者: {from_wxid}, 名片ID: {card_wxid}, 名片名称: {card_name}")
        else:
